...
-->
            <div class="row">
                <button @click="validate" class="calculator_submit-btn">
                    Отправить заявку
                </button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        validate() {
            ...
            for (const key of Object.keys(vm.formErrors)) {
                if (key !== "coBorrowerPercent") {
                    ...
                    if (
                        vm.formData.haveCoBorrower.value === "Да" &&
                        !vm.formData[key].value.length
                    ) {
                        errorsCount++;
                        vm.formErrors[key] = true;
                    } else vm.formErrors[key] = false;
        ...
        submit(requestBody) {
            window.axios.post("/hypothec-insurance/form", requestBody);
            this.$emit("onSuccess");
        },
    },
    components: {
