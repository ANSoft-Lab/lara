<section class="offer-links">
    <a href="https://ipoteka-strahovka.ru/" class="offer-links__item">
        <div class="offer-links__wrap">
            <img src="/img/offers/1.jpg" alt="" class="offer-links__img">
        </div>
        <p class="offer-links__text{{ isset($edit_mode) ? ' blockcrud-editable' : '' }}">Комплексное ипотечное страхование</p>
        <div class="offer-links__arrow"></div>
    </a>
    <a href="https://refin.finist.ru" target="_blank" class="offer-links__item">
        <div class="offer-links__wrap">
            <img src="/img/offers/2.jpg" alt="" class="offer-links__img">
        </div>
        <p class="offer-links__text{{ isset($edit_mode) ? ' blockcrud-editable' : '' }}">Рефинанси&shy;рование и кредитование</p>
        <div class="offer-links__arrow"></div>
    </a>
    <a href="{{ config('app.old_url') }}/strahovanie/kasko/" target="_blank" class="offer-links__item">
        <div class="offer-links__wrap">
            <img src="/img/offers/3.jpg" alt="" class="offer-links__img">
        </div>
        <p class="offer-links__text{{ isset($edit_mode) ? ' blockcrud-editable' : '' }}">Автострахова&shy;ние</p>
        <div class="offer-links__arrow"></div>
    </a>
    <a href="{{ config('app.old_url') }}/strahovanie/strahovanie_imuchestva/" target="_blank" class="offer-links__item">
        <div class="offer-links__wrap">
            <img src="/img/offers/4.jpg" alt="" class="offer-links__img">
        </div>
        <p class="offer-links__text{{ isset($edit_mode) ? ' blockcrud-editable' : '' }}">Страхование имущества</p>
        <div class="offer-links__arrow"></div>
    </a>
</section>
