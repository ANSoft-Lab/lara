...
<?php
    $field['options'] = [
        ...
        'old_link' => trans('backpack::base.menu_model.old_link'),
    ];
...
@include('crud::fields.inc.wrapper_start')
    ...
    echo 'd-none';
              ...
              <!-- old link input -->
              <div class="page_or_link_value page_or_link_old_link <?php if (! isset($entry) || $entry->$entry_type != 'old_link') {
    echo 'd-none';
} ?>">
                <input
                    type="text"
                    class="form-control"
                    name="{!! $entry_link !!}"
                    placeholder="{{ trans('backpack::base.menu_model.old_link_placeholder', ['url', url(config('backpack.base.route_prefix').'/page')]) }}"

                    @if (!isset($entry) || $entry->$entry_type!='old_link')
                        disabled="disabled"
                      @endif

                    @if (isset($entry) && $entry->$entry_type=='old_link' && isset($entry->$entry_link) && $entry->$entry_link!='')
                        value="{{ $entry->$entry_link }}"
                    @endif
                    >
              </div>
            ...
            function bpFieldInitPageOrLinkElement(element) {
                element.find('[data-identifier=page_or_link_select]').change(function(e) {
                    ...
                    switch($(this).val()) {
                        ...
                        case 'old_link':
                            $(this).closest('.row').find(".page_or_link_old_link input").removeAttr('disabled');
                            $(this).closest('.row').find(".page_or_link_old_link").removeClass('d-none');
                            break;
