...
class Bitrix24Service {
    ...
    public static function sendLead($order) {
        ...
        $postData = [
            ...
            'TITLE' => ("Партнер с сайта {$order['referer']}" ?? 'Партнёр с сайта') . " " . date("d.m.Y H:i") . " ({$params['fullName']})", // заголовок для лида
...
//            DB::table('order_requests')->where('id', $order->id)->update(['bitrix24_res' => json_encode($json), 'bitrix24_lead_id' => @$json->ID]);
    ...
    private static function prepareFields($params) {
        ...
        if ($params['source'] == '') {
            $params['source'] = "Партнер с сайта (API) " . ($params['referer'] ?? '-');
        }
...
/*
    private static function sendToDB($order) {
        ...
        if(isset($order['bitrix24_res'])) {
            $sql['bitrix24_res'] = $order['bitrix24_res'];
        }

        $new_id = DB::table('order_requests')->insertGetId($sql);
        return $new_id;
    }

    private static function updateLeadInDB($id, $bitrix24_lead_id, $bitrix24_res) {
        if(!intval($id)) return;
        DB::table('order_requests')->where('id', intval($id))->update(['bitrix24_lead_id'=>$bitrix24_lead_id, 'bitrix24_res'=>$bitrix24_res]);
    }
*/
